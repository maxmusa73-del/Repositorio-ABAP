...
*    LOOP AT me->passenger_flights INTO DATA(flight)   " no queremos que se recorra toda la tabla solo esto
    LOOP AT me->flights INTO DATA(flight)
        WHERE table_line->flight_date >= i_from_date
      ...
      IF connection_details-airport_from_id = i_airport_from_id
       AND connection_details-airport_to_id = i_airport_to_id
*       AND flight->get_free_seats( ) >= i_seats.  " dentro del padre
...
*             / lines( passenger_flights ) .
    ...
    r_result = REDUCE #(
                 ...
                 WHERE ( table_line IS INSTANCE OF lcl_passenger_flight )  " solo quiero procesar las filas de pasajeros
                 NEXT i += CAST lcl_passenger_flight( flight )->get_free_seats( )  " para poder utilizar el método del hijo
                       ) / pf_count.
