CLASS zcl_mms_solution_24 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_MMS_SOLUTION_24 IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    CONSTANTS c_carrier_id TYPE /dmo/carrier_id VALUE 'XX'. " cambiamos LH por XX y por UA
...
*      CATCH cx_abap_invalid_value.  "catch sirve para capturar excepciones y poder tratarla y siempre se mete entre TRY y ENDTRY
       CATCH cx_abap_invalid_value INTO DATA(exc_inv). " la capturamos en exc_inv
*        out->write( | Carrier { c_carrier_id } does not exist | ).
         out->write( exc_inv->get_text( ) ). " para que escriba el texto real de la excepción
